...
public interface AppraiseVehicleRepo extends JpaRepository<EAppraiseVehicle,Long>, JpaSpecificationExecutor<EAppraiseVehicle> {
   ...
   // @Query("select e from EAppraiseVehicle e where e.vinNumber=:vin and e.dealer.id=:dealerId and e.valid=true")
    ...
    @Query(value = "SELECT av FROM EAppraiseVehicle av " +
            ...
            "AND av.id NOT IN (SELECT o.appRef FROM EOffers o WHERE o.status in (4,5) AND o.appRef IN (SELECT av.id FROM EAppraiseVehicle av WHERE av.user.id <> :userId AND av.invntrySts = :inventory AND av.valid = true)) " +
 ...
 @Query(value = "select e from EAppraiseVehicle e where e.valid=true ")
 List<EAppraiseVehicle> findAllCards();

 @Query("SELECT DISTINCT v.vehicleMake FROM EAppraiseVehicle v")
 List<String> findDistinctMakes();
 @Query("SELECT DISTINCT v.vehicleModel FROM EAppraiseVehicle v")
 List<String> findDistinctModels();
 @Query("SELECT DISTINCT v.vehicleSeries FROM EAppraiseVehicle v")
 List<String> findDistinctSeries();
 @Query("SELECT DISTINCT v.tdStatus.engineType FROM EAppraiseVehicle v")
 List<String> findDistinctEngine();
