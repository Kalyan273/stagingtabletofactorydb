...
//            EDealerRegistration dealerById = dealerRepo.findDealerById(id);
            ...
            /*else if (null != dealerById) {
                pageResult = offersRepo.findByBuyerDealerIdJPQL(id, AppraisalConstants.INVENTORY, true, false, pageable);
...
//            EDealerRegistration dealerById = dealerRepo.findDealerById(id);
         ...
         /*  else if (null != dealerById) {
                pageResult = offersRepo.findBySellerDealerIdJPQL(id, false, pageable);
            }*/
            if (null!= pageResult&&pageResult.getTotalElements() != 0) {
    ...
    public Response makeOfferByBuyer(Long appraisalId, Offers offers, UUID buyerUserId) throws OfferException {
        ...
        if(checkvalue(offers.getBuyerQuote())){
    ...
    public Response sellerCounter(Long offerId, Offers offers) throws OfferException {
        ...
        if(checkvalue(offers.getSellerQuote())){
            if (null != offerById) {
    ...
    public Response myScheduledTask() throws IOException, TemplateException, MessagingException {
        ...
        System.out.println("myScheduledTask() method started..");
        Logger log = LoggerFactory.getLogger(OffersServiceImpl.class);
        Response response = new Response();
...
//        offerQuotes.setSellerQuote(0.0);
    ...
    private void insertShipmentDtls(EOffers eOffers ){
